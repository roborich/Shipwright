...
| **Browser** | Branch `unbound-wasm` merges the `wasm` port: the same game as a WebAssembly module for Prelude's site. It boots from `oot-unbound.o2r` alone (standalone base: checked against the running converter, never re-derived), or converts a supplied `oot.o2r` at startup and hands the result to the page as a `file-saved` event; `warp` and `entrance` take entrance names so a page can boot into a custom scene. | [`soh/wasm/HOST-API.md`](../soh/wasm/HOST-API.md), [`wasm-port.md`](../wasm-port.md) | §6 |
