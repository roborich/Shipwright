...
| Epona in a custom scene with `horse`: play Epona's Song, ride out through an exit into another horse scene, dismount + save + reload | `SceneDB_HorseAllowed` gate, `OBJECT_HORSE` injected per room, generated call point, parked scene saved by name | she comes onto flat dry ground — from off camera where the scene has anywhere off camera — stays under Link across the transition, and is where she was left |
| Open the Better Debug Warp screen with a custom-scene mod mounted; warp to each of a custom scene's entrances as child and adult, day and night | `UnboundSceneSelect_BuildList` (registry scenes appended to `sBetterScenes`), custom entrance indices through `Select_LoadGame` | the scene is listed after `50:Debug` by its display name and every entrance lands at its spawn |
