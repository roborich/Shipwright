...
import { exportNames, functionBodies, largestFunctions, mostLocals, readLeb128 } from "../lib/wasm";

// One imported function ("m"."f"), two defined functions named "a" (2-byte body) and "b"
// (4-byte body, three i32 locals), "b" exported.
const MODULE = new Uint8Array([
    ...
    0x0a, 0x09, 0x02, 0x02, 0x00, 0x0b, 0x04, 0x01, 0x03, 0x7f, 0x0b, // code
...
test("functionBodies indexes past imports and applies the name section", () => {
    expect(functionBodies(MODULE)).toEqual([
        { index: 1, name: "a", size: 2, locals: 0 },
        { index: 2, name: "b", size: 4, locals: 3 },
    ]);
    expect(largestFunctions(MODULE, 1)).toEqual([{ index: 2, name: "b", size: 4, locals: 3 }]);
    expect(mostLocals(MODULE, 1).map((f) => f.name)).toEqual(["b"]);
});
