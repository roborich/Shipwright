...
// A config from a Mac install names "coreaudio". The browser build has only the SDL player,
// and LUS used to fall through to the null player for any backend it was not built with, so
// the game ran silent with nothing in the console.
test("a config naming a backend this build lacks still plays audio", () => {
